...
void test_allbutton_ack_encoding() {
    uint8_t encoded[MAX_FRAME_SIZE] = {};
    const size_t length =
        encodeAck(ACK_ALLBUTTON, 0x00, encoded, sizeof(encoded));
    const uint8_t expected[] = {
        NUL, DLE, STX, 0x00, CMD_ACK, 0x80, 0x00, 0x93, DLE, ETX, NUL,
    };
    TEST_ASSERT_EQUAL_UINT32(sizeof(expected), length);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(expected, encoded, sizeof(expected));
}
...
int main(int, char**) {
    ...
    RUN_TEST(test_allbutton_ack_encoding);
